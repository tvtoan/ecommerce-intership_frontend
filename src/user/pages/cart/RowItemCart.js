...
export default function RowItemCart({itemCart, onRemove, ...props}) {
  return (
    <tr className="cart-page__list-item">
      <td className="cart-page__list-item__product">
        <div className="block-product">
          <img src={`${process.env.REACT_APP_SERVER_URL}/${itemCart.product.coverImage.path}`} alt="" className="block-product__image" />
          <div className="block-product__info">
            ...
            <div className="block-product__method">
              ...
              <button className="flat-button" onClick={() => onRemove(itemCart)}>Remove</button>
            </div>
          </div>
        </div>
      </td>
      <td className="cart-page__list-item__color">
        <div className="block-color" style={{backgroundColor: itemCart.color.code}}></div>
      </td>
      <td className="cart-page__list-item__size">{itemCart.size.name}</td>
      <td className="cart-page__list-item__quantity">
        <Touchspin value={itemCart.quantity}/>
      </td>
      <td className="cart-page__list-item__amount">${itemCart.product.price}</td>
    </tr>
  );
}
